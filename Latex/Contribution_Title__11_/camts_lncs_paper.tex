...
\begin{center}
\includegraphics[width=0.97\textwidth,height=0.32\textheight,keepaspectratio]{plasmaxai_framework.png}
\captionof{figure}{End-to-end PlasmaXAI workflow from image preprocessing to explainable prediction.}\label{fig:workflow}
\end{center}
\noindent Figure~\ref{fig:workflow} outlines the full processing pipeline used to generate predictions, explanations, and patient-level outputs.

\begin{center}
\includegraphics[width=0.97\textwidth,height=0.34\textheight,keepaspectratio]{Architecture_diagram.png}
\captionof{figure}{Detailed internal PlasmaXAI fusion architecture with image, morphology, and counterfactual branches.}\label{fig:archdetail}
\end{center}
\noindent Figure~\ref{fig:archdetail} emphasizes that counterfactual features are inserted into the fusion process itself rather than appended only after classification.

\subsection{Training and Evaluation}
The deployment operating point was selected on validation data using a precision-recall balanced criterion. Final evaluation reports accuracy, weighted F1, plasma precision, plasma recall, and AUC. The study also includes ablation, bootstrap confidence intervals, calibration curves, and patient-level interpretation analysis.
...
\begin{center}
\includegraphics[width=0.97\textwidth,height=0.31\textheight,keepaspectratio]{plasmaxai_model_comparison.png}
\captionof{figure}{Expanded benchmark comparison showing PlasmaXAI against deep and morphology-based baselines.}\label{fig:modelcomp}
\end{center}
\noindent Figure~\ref{fig:modelcomp} shows that the full framework delivers the strongest balance across the main benchmark metrics rather than improving one measure at the expense of another.
...
\begin{center}
\includegraphics[width=0.96\textwidth,height=0.29\textheight,keepaspectratio]{plasmaxai_counterfactual.png}
\captionof{figure}{Counterfactual explanation analysis for PlasmaXAI.}\label{fig:counterfactual}
\end{center}
\noindent Figure~\ref{fig:counterfactual} shows the feature-shift directions and modality-gating behavior that support a malignant decision.

\begin{center}
\includegraphics[width=0.96\textwidth,height=0.29\textheight,keepaspectratio]{plasmaxai_ablation.png}
\captionof{figure}{Ablation study showing the effect of removing key PlasmaXAI components.}\label{fig:ablation}
\end{center}
\noindent Figure~\ref{fig:ablation} shows that removing the counterfactual branch causes the largest drop in malignant-cell recall.
...
\subsection{Reliability and Patient-Level Interpretation}
Internal reliability analysis remained favorable, and the patient-level patterns remained visually consistent across the exploratory cohort. Even so, the patient-level result should be interpreted carefully because the cohort is small and internally sourced. It is therefore presented as supportive evidence rather than definitive clinical validation.

\begin{center}
\includegraphics[width=0.96\textwidth,height=0.28\textheight,keepaspectratio]{plasmaxai_calibration_bootstrap.png}
\captionof{figure}{Calibration curve and bootstrap stability analysis for PlasmaXAI.}\label{fig:calibration}
\end{center}
\noindent Figure~\ref{fig:calibration} shows that the selected operating point remains well behaved under repeated resampling.

\begin{center}
\includegraphics[width=0.96\textwidth,height=0.28\textheight,keepaspectratio]{plasmaxai_clinical_insight.png}
\captionof{figure}{Clinical insight diagram summarizing the dominant disease-signature patterns discovered by PlasmaXAI.}\label{fig:clinical}
\end{center}
\noindent Figure~\ref{fig:clinical} summarizes the morphologic and chromatic patterns most associated with the malignant decision path.

\begin{center}
\includegraphics[width=0.96\textwidth,height=0.28\textheight,keepaspectratio]{plasmaxai_patient_heatmap.png}
\captionof{figure}{Patient-level heatmap showing the consistency of disease-related morphology patterns across cases.}\label{fig:heatmap}
\end{center}
\noindent Figure~\ref{fig:heatmap} makes the patient-wise pattern consistency easier to inspect than a single scalar performance score.
