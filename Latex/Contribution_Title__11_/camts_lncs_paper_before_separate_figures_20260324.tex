...
\begin{abstract}
Recognizing malignant plasma cells is important for multiple myeloma screening, yet manual microscopic review remains slow and subjective. This paper presents PlasmaXAI, an explainable digital pathology framework that combines frozen ResNet50 and DenseNet121 embeddings with handcrafted morphology, a morphology-derived counterfactual boundary model, and learned fusion gates that inject counterfactual information directly into the final decision pathway. Evaluated on PCMMD, PlasmaXAI achieved 93.42\% accuracy, 93.43\% weighted F1, 91.24\% plasma precision, 94.63\% plasma recall, and 97.76\% AUC at the selected operating point. The framework also produced counterfactual explanation panels, calibration and bootstrap analysis, ablation evidence, and exploratory patient-level robustness results. These findings position PlasmaXAI as an interpretable screening framework rather than a purely black-box classifier, with practical relevance for digital hematopathology workflows that require both strong discrimination and clinically readable reasoning.
...
Multiple myeloma is a hematologic malignancy characterized by abnormal plasma-cell proliferation in bone marrow, and morphologic assessment remains central to diagnosis, disease monitoring, and treatment planning \cite{mmreview}. In many laboratories, stained marrow smears are still reviewed manually. This workflow is labor-intensive, sensitive to stain variation, and vulnerable to reader-to-reader disagreement, especially when case volume is high and subspecialty review time is limited.

Recent progress in computational pathology suggests that deep learning can reduce this burden, but strong accuracy alone is not sufficient for clinical use \cite{litjens,bera,campanella}. A clinically useful system for plasma-cell analysis should support review rather than replace it; therefore, it must expose why it predicts malignancy, which features would change the decision, and how stable that decision is under resampling and patient aggregation.
...
These signals are passed to a learned fusion module that separately encodes ResNet, DenseNet, morphology, counterfactual, and score streams. Counterfactual features modulate the image branches through sigmoid gates and also contribute to a softmax-based modality gate that controls the final weighted fusion. This design makes counterfactual reasoning structurally relevant to prediction rather than merely descriptive after prediction. Figure~\ref{fig:arch} presents the high-level workflow and the internal architecture.

\begin{figure}[t]
\centering
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.22\textheight,keepaspectratio]{plasmaxai_framework.png}\\[2pt]
\small (a) End-to-end PlasmaXAI workflow.
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.22\textheight,keepaspectratio]{Architecture_diagram.png}\\[2pt]
\small (b) Internal fusion configuration.
\end{minipage}
\caption{PlasmaXAI from input processing to counterfactual-guided fusion.}
\label{fig:arch}
\end{figure}
Figure~\ref{fig:arch} shows how image embeddings, morphology descriptors, counterfactual features, and learned gates are connected inside the final classifier.

\subsection{Training and Evaluation}
The deployment operating point was selected on validation data using a precision-recall balanced criterion. Final evaluation reports accuracy, weighted F1, plasma precision, plasma recall, and AUC. The study also includes ablation analysis, bootstrap confidence intervals, calibration curves, exact patient-level permutation testing, and leave-one-patient-out robustness checks to examine how stable the learned decision policy remains beyond a single split.
...
\bottomrule
\end{tabular}\\[2pt]
\small (a) Held-out test metrics (\%).
\end{minipage}\hfill
\begin{minipage}[t]{0.56\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.22\textheight,keepaspectratio]{plasmaxai_model_comparison.png}\\[2pt]
\small (b) Expanded benchmark comparison.
\end{minipage}
\caption{Benchmark evidence showing the overall advantage of PlasmaXAI.}
\label{fig:benchmark}
\end{figure}
Figure~\ref{fig:benchmark} shows that the full framework delivers the strongest balance across the main benchmark metrics rather than improving one measure at the expense of another.

Bootstrap analysis indicated that the selected operating point was stable, with a 95\% confidence interval of 91.16--95.30 for accuracy, 91.30--97.20 for plasma recall, and 96.56--98.63 for AUC. This matters clinically because a reliable screening aid must remain strong under resampling rather than depending on a single favorable split.
...
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.20\textheight,keepaspectratio]{plasmaxai_counterfactual.png}\\[2pt]
\small (a) Counterfactual explanation analysis.
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.20\textheight,keepaspectratio]{plasmaxai_ablation.png}\\[2pt]
\small (b) Ablation study.
\end{minipage}
\caption{Embedded explainability and component-wise ablation analysis.}
\label{fig:xaiablation}
\end{figure}
Figure~\ref{fig:xaiablation} shows that explainability is not merely appended after inference; it is embedded in the decision path, and removing the counterfactual branch causes the largest drop in malignant-cell recall.
...
Internal reliability analysis remained favorable. Exact permutation testing on the exploratory patient cohort produced p-values of 0.0040 (one-sided) and 0.0079 (two-sided), while the leave-one-patient-out patient AUC remained 1.000. Even so, this result should be interpreted carefully because the cohort is small and internally sourced. The patient-level analysis is therefore presented as an encouraging signal of separability and consistency, not as definitive clinical validation.

\begin{figure}[t]
\centering
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.16\textheight,keepaspectratio]{plasmaxai_calibration_bootstrap.png}\\[2pt]
\small (a) Calibration and bootstrap stability.
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.16\textheight,keepaspectratio]{plasmaxai_patient_robustness.png}\\[2pt]
\small (b) Patient-level robustness checks.
\end{minipage}\\[4pt]
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.16\textheight,keepaspectratio]{plasmaxai_clinical_insight.png}\\[2pt]
\small (c) Clinical insight analysis.
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.16\textheight,keepaspectratio]{plasmaxai_patient_heatmap.png}\\[2pt]
\small (d) Patient-level heatmap.
\end{minipage}
\caption{Reliability analysis and patient-level interpretation produced by PlasmaXAI.}
\label{fig:reliability}
\end{figure}
Figure~\ref{fig:reliability} shows that the selected operating point is well calibrated and statistically stable, while also summarizing the patient-level morphology patterns learned by the framework.

Taken together, the results position PlasmaXAI as an explainable screening system rather than a narrowly optimized classifier. Its value lies in combining strong discrimination with traceable counterfactual reasoning, uncertainty summaries, and patient-wise consistency analysis that a clinician can inspect and challenge.
