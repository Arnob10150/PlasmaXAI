...
\begin{abstract}
Reliable plasma cell assessment is central to multiple myeloma diagnosis, yet manual microscopic review is labor-intensive, subjective, and difficult to scale in resource-constrained laboratories. This paper presents PlasmaXAI, an explainable computational pathology framework for malignant plasma cell recognition using a counterfactual-guided learned fusion model. PlasmaXAI combines frozen ResNet50 and DenseNet121 image embeddings with handcrafted morphology, a morphology-derived counterfactual boundary model, and a learned gating mechanism that injects counterfactual signals directly into the decision path. The framework is evaluated on the PCMMD dataset with an internal patient-level analysis pipeline. At the selected precision-recall balanced operating point, PlasmaXAI achieves 93.42\% accuracy, 93.43\% weighted F1, 91.24\% plasma precision, 94.63\% plasma recall, and 97.76\% AUC, outperforming deep and morphology-based baselines. Beyond classification, the model provides counterfactual feature analysis, calibration and bootstrap confidence summaries, ablation evidence, and exploratory patient-level robustness results. The study positions PlasmaXAI as a practical explainable screening framework rather than a black-box classifier, with particular relevance to digital hematopathology workflows that need both performance and transparent reasoning.
...
\subsection{Deep Learning for Digital Pathology}
...
\subsection{Explainable AI and Counterfactual Reasoning}
...
These counterfactual features are then passed into a learned fusion model. Separate encoders process the ResNet, DenseNet, morphology, counterfactual, and score streams. Counterfactual signals modulate the image features through sigmoid gates, refine the score branch, and contribute to a softmax modality gate that controls the final fusion weights. This design makes the counterfactual pathway structurally important rather than decorative. The overall framework is shown in Fig.~\ref{fig:framework}.

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{plasmaxai_framework.png}
\caption{PlasmaXAI architecture showing image embeddings, morphology features, counterfactual-guided fusion, and explainability outputs.}
\label{fig:framework}
\end{figure}
...
Patient-level analysis produced an internal AUC of 1.000 on a ten-patient cohort, with exact permutation p-values of 0.0040 one-sided and 0.0079 two-sided. This is encouraging, but it is still not external clinical proof because the cohort is small and internally sourced. The calibration summary in Fig.~\ref{fig:calib} and the robustness visualization in Fig.~\ref{fig:patient} therefore serve as cautionary context for the otherwise strong internal separation.
...
Taken together, the results position PlasmaXAI as a strong explainable classifier for plasma cell recognition. Its practical advantage is not only improved classification, but the fact that model behavior can be interrogated through counterfactual features, uncertainty summaries, and patient-level consistency analysis.

\section{Conclusion}
This paper replaced the CAMTS-UNet manuscript with a new LNCS paper focused on PlasmaXAI, an explainable fusion framework for malignant plasma cell recognition. PlasmaXAI integrates deep image embeddings, morphology descriptors, and counterfactual reasoning into a unified learned decision path and achieves strong held-out performance while preserving interpretability. The model outperforms key baselines, shows meaningful sensitivity to removal of the counterfactual branch, and provides transparent diagnostic figures that support auditing and future clinician-in-the-loop validation. The main limitation is the lack of external patient-level validation. Future work should therefore focus on broader hematopathology cohorts, stain-robust generalization, and prospective evaluation in clinical laboratory workflows.
