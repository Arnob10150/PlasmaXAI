...
def main():
    parser = argparse.ArgumentParser(description="Run the novel PlasmaXAI model on a single cell image.")
    parser.add_argument("image_path", help="Path to a single cell image.")
    args = parser.parse_args()

    summary = json.loads((ROOT / "novel_outputs" / "novel_summary.json").read_text(encoding="utf-8"))
    scalers = joblib.load(ROOT / "novel_outputs" / "fusion_scalers.joblib")
    cf_bundle = joblib.load(ROOT / "novel_outputs" / "counterfactual_bundle.joblib")
    feature_blocks = scalers["feature_blocks"]
    config = summary["best_fusion_config"]

    resnet = load_backbone("resnet50", ROOT / "optimization_outputs" / "checkpoints" / "resnet50_final.pth")
    densenet = load_backbone("densenet121", ROOT / "optimization_outputs" / "checkpoints" / "densenet121_final.pth")
    ...
    model.load_state_dict(torch.load(ROOT / "novel_outputs" / "novel_fusion_model.pth", map_location=DEVICE))
    model.eval()
    ...
    threshold = float(summary.get("novel_threshold", 0.5))
